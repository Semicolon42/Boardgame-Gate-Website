...
export function GameStatsView({isActive}: Props) {
	const [records, setRecords] = useState<GameRecord[]>(() => loadGameRecords())

	// Refresh from localStorage each time the view becomes visible
	useEffect(() => {
		if (isActive) setRecords(loadGameRecords())
	}, [isActive])
				...
				<div className='overflow-x-auto'>
					<table className='w-full text-xs border-collapse'>
						...
						<tbody>
							{records.map((r, i) => (
								<tr
									className={r.outcome === 'WIN' ? 'bg-green-50' : 'bg-red-50'}
									key={i}
								>
